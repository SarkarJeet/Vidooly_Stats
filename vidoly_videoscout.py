...
def FIT(train, batch_size, epoch, neuron):
    ...
    model = Sequential()
    model.add(LSTM(neuron, batch_input_shape=(
...
# Loading dataset
DataSet = pd.read_csv(
    "/Users/dd250050/Downloads/channel_stats/data/oct_march.CSV", usecols=[0, 3])

DataSet.tail()
DataSet = DataSet[(DataSet.chid == 'UC64do0QynUFrMe85Zg')]


DataSet = DataSet.drop(['chid'], axis=1)

# Loading the last two values of the dataset for LSTM to use
FinalInput = [1644148, 1644703]

# Transforming data to be stationary
Data_array = DataSet.values
Diff_array = diffSe(Data_array, 1)
FinalInput_Diff_array = diffSe(FinalInput, 1)

# Transforming data to be supervised
supArray = sup(Diff_array, 1)
supArray_values = supArray.values
FinalInput_supervised = sup(FinalInput_Diff_array, 1)
FinalInput_supervised_values = FinalInput_supervised.values

# Renaming Data into train and FinalInput-sets
train = supArray_values
...
# Fitting the model
LSTMModel = FIT(train_scaled, 1, 2000, 4)
...
for i in range(90):
    ...
    # storing forecast
    New_predictions = New_predictions.append(
        pd.DataFrame({'SubCount': int(OP)}, index=[i + 1]))

    Prev_value = OP  # Changing last value to the current value for the next iteration


print(New_predictions)
New_predictions['chid'] = 'UC64do0QynUFrMe85Zg'
New_predictions.to_csv(
    "SubscriberCount_UC64do0QynUFrMe85Zg_views.csv", index=False)
